...
def grad_descent(step_fn, env):
    ...
    status = env.status()
    while status == Environment.ACTIVELY_SEARCHING:
        curr_pos = env.current_position()
        step = step_fn(curr_pos)
        new_pos = curr_pos + step
        status = env.step_to(new_pos)
        positions.append(new_pos)
        if status != Environment.ACTIVELY_SEARCHING:
            break
    return positions
...
class MomentumStepFunction:
    ...
    def __call__(self, pos):
        ...
        new_state = self.momentum_rate * self.state - self.learning_rate * grad
...
class AdagradStepFunction:
    ...
    def __call__(self, pos):
        self.gradients.append(self.loss_gradient(pos))
        return -self.learning_rate * self.loss_gradient(pos) / (self.delta + torch.sqrt(sum([gradient**2 for gradient in self.gradients])))
